...
def threaded_process(j, json_lines, role):
    gpt = GPT(model_name='gpt-4', user_name='ZhangSan', use_vpn=False, p_key="sk-xxx")
...
# 循环处理每个批次
for start_index in range(17600, 24052, batch_size):
    threads = []
    # 为当前批次的每个数据创建线程
    for j in range(start_index, min(start_index + batch_size, 24052)):
        thread = threading.Thread(target=threaded_process, args=(j, json_lines, role))
